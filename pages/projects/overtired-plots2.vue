  ...
  <div class="pb-10">
    ...
    <transition name="fade">
          ...
          <transition name="fade">
            <div v-if="isGridView" key="table" ref="gridRef" class="absolute">
              <div class="grid grid-cols-3 gap-4">
                <div v-for="img in items" :key="img" class="rounded-lg overflow-hidden">
                  <NuxtImg :src="`images/${img}`" width="300" height="162" format="webp" class="w-full rounded-xl "
                    draggable="false" />
                </div>
              </div>
            </div>
      ...
      class: 'invisible group-hover:visible scale-0 group-hover:scale-100 transition-transform'
                ...
                <NuxtImg :src="`images/${item}`" width="1000" format="webp" class="w-full rounded-xl"
                  draggable="false" />
              </UCarousel>
            </div>
          </transition>
        ...
        <div class="grid lg:grid-cols-4 grid-cols-1 mt-3 gap-y-3 lg:gap-3">
          <div
            ...
            <p>
              Organised around an interview with historian Sabine Hörler and pulling
              together footage from forest research facilities this video explores
