<template>
  <div class="hidden">
    <div v-for="img in items" :key="img" class="rounded-lg overflow-hidden">
      <NuxtImg :src="`images/${img}`" format="webp" width="300" height="162" class="w-full" draggable="false" />
    </div>
    <div v-for="img in items" :key="img" class="rounded-lg overflow-hidden">
      <NuxtImg :src="`images/${img}`" width="1000" format="webp" class="w-full" draggable="false" />
    </div>
  </div>
  <div class="pb-10">
      ...
      <transition name="fade">
        <div v-if="isGridView" key="grid" ref="gridRef" class="absolute">
          <div class="grid grid-cols-3 gap-4">
            <div v-for="img in items" :key="img" class="rounded-lg overflow-hidden shadow-md">
              <NuxtImg :src="`images/${img}`" format="webp" width="300" height="162" class="w-full" draggable="false" />
            </div>
          </div>
        </div>
    ...
    <div class="grid lg:grid-cols-4 grid-cols-1 mt-3 gap-y-3 lg:gap-3">
      <div
        ...
        <p>
          Organised around an interview with historian Sabine Hörler and pulling together footage from forest research
          facilities this video explores how the technologisation of forest environments conditions how we relate to
          them.
        </p>
      </div>
      <div
